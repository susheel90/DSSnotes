...
png("plot4.png", width = 480, height = 480, units = "px")
    ...
    #graph 2
    with(power.data, plot(power.data$Date_Time, power.data$Voltage, type = "n", xlab = "datetime", ylab = "Voltage"))
    ...
    with(subset(new.data, variable == "Sub_metering_3"), lines(Date_Time, value, col = "blue", type = "l"))
        legend("topright", legend = c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"), text.col = c("black", "red", "blue"), border = "white")

    #graph 4    
    with(power.data, plot(Date_Time, Global_reactive_power, type = "n", xlab = "datetime", ylab = "Global_reactive_power"))
    lines(power.data$Date_Time, power.data$Global_reactive_power)
